...
CREATE TABLE transactions (
    ...
    -- Генеративные колонки для быстрого доступа к частям даты
    created_date TEXT GENERATED ALWAYS AS (
        strftime('%Y-%m-%d', datetime(created_at/1000, 'unixepoch'))
    ) VIRTUAL,
    created_year INTEGER GENERATED ALWAYS AS (
        CAST(strftime('%Y', datetime(created_at/1000, 'unixepoch')) AS INTEGER)
    ) VIRTUAL,
    created_month INTEGER GENERATED ALWAYS AS (
        CAST(strftime('%m', datetime(created_at/1000, 'unixepoch')) AS INTEGER)
    ) VIRTUAL,
    created_day INTEGER GENERATED ALWAYS AS (
        CAST(strftime('%d', datetime(created_at/1000, 'unixepoch')) AS INTEGER)
    ) VIRTUAL,
...
-- Индексы для ускорения запросов по датам
